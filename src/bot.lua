...
cmds["verify"] = function(args, msg)
    ...
    if not res then
        ...
        local m = msg.channel:send("We encountered an error while sending the DM... check if your username is correct and try again later!")
    ...
    timer.setTimeout(1000 * 60 * 5, coroutine.wrap(function() -- expire the key in 5 minutes
        ...
        verificationKeys[msg.author.id] = nil
        local m = msg.channel:send("<@" .. msg.author.id .. "> Verification process timed out! Please do !verify again")
        ...
        if keys then
            ...
            if keys.key == msg.content then
                ...
                if keys.attempts < 1 then
                    verificationKeys[msg.author.id] = nil
                    local m = msg.channel:send("Verification failed!!! Please try again later")
                    return timer.setTimeout(1000 * 60 * 1, coroutine.wrap(function() m:delete() end))
                end
                local m = msg.channel:send("Wrong key! **" .. keys.attempts .. "** attempts left!")
                timer.setTimeout(1000 * 60 * 1, coroutine.wrap(function() m:delete() end))
